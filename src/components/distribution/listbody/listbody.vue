								...
								<table class="table table-striped table-hover table-bordered">
                                     <thead>
									     <tr class="text-center">
			            				    ...
			            				    <th class="text-center">配送区域编码</th>
			            				    <th class="text-center">配送区域名称</th>
			            				    <th class="text-center">配送区域缩写</th>
			            				    <th class="text-center">配送区域业务值</th>
						...
						<div class="modal-dialog modal-wide" >
							<div class="modal-content">
								<div class="modal-header">
									...
									<h4 class="modal-title text-center">新增配送区域管理</h4>
								</div>
								<div class="modal-body form clearfix">
									<div class="col-md-5 col-sm-6 tree-padding">
										<ZTree ref='tree' :treeData="treeData" :options="options" @node-click="addClick" @add-node="addNode" :key="2">
										</ZTree>
									</div>
					...
					<div class="modal fade" id="updata" aria-hidden="true">
						<div class="modal-dialog modal-wide">
							<div class="modal-content">
								<div class="modal-header">
									<button type="button" class="close" data-dismiss="modal" aria-hidden="true"></button>
									<h4 class="modal-title text-center">编辑配送区域管理</h4>
								</div>
								<div class="modal-body form clearfix">
									<div class="col-md-5 col-sm-6 tree-padding">
										<ZTree ref='tree'
	...
	export default {
		...
		data() {
			return {
				...
				treeRootCode: commonConfig.areaRoot.darea,    //点击树根节点获取的相关编码
				areaCode: '',
				checked: false,
				obj: {},
				radioValue: -1,
		...
		methods: {
			//初始化缓存
			init() {
				api.initDeliveryAreaToCache().then((res) => {
			...
			updata() {
				...
				api.getUpdateDeliveryAreaInfo(_params).then((res) => {
					  if( res.data.code === 'success' ) {
			...
			addData() {
				...
				let insertPromise = new Promise((resolve, reject) => {
					...
					api.getInsertDeliveryAreaInfo(_params).then((res) => {
						if( res.data.code === 'success' ) {
							resolve(_areaCode);
			...
			add() {
				...
				this.required = false
				this.getTree();
				let _params = commonConfig.area.darea;
				api.ordinalInfo.getSequence({"serviceCode":_params}).then((res) => {
					this.addDataParams.areaCode = res.data.obj;
				});
				this.addDataParams.fatherAreaCode = commonConfig.areaRoot.darea;
			},
			//根据配送区域编码返回其直属下级
			getTree() {
                let _params = commonConfig.areaRoot.darea;
				let _this = this;
				api.getDeliveryAreaInfoByAreaCode({"areaCode" : _params}).then(function(res) {
                      if( res.data.code === 'success' ) {
							if(res.data.obj) {
								_this.treeData = [];
								_this.options.rootName =  res.data.obj.areaName;
								let deliveryAreaSubInfo = res.data.obj.deliveryAreaSubInfo;
								if(deliveryAreaSubInfo) {
									deliveryAreaSubInfo.forEach((item,index) => {
										let treeData = {
											label: item.areaName,
			...
			change(id) {
				if( this.checked ) {
					...
					api.getQueryDeliveryAreaInfo({"areaCode":_params}).then((res) => {
			...
			getQueryInfo(params) {
				this.radioValue = -1
				api.getQueryDeliveryAreaInfo(params).then((res) => {
					if(res.data.code === 'success') {
			...
			getRootUpdata(rootCode) {
				this.updataParams.fatherAreaCode = commonConfig.areaRoot.darea;
			},
			getRootAdd(rootCode) {
				this.addDataParams.fatherAreaCode = commonConfig.areaRoot.darea;
			},
			...
			async loadingChild(node, index) {
				...
				let data = await new Promise((resolve, reject) => {
					console.log(node.areaCode)
						api.getDeliveryAreaInfoByAreaCode({"areaCode" : node.areaCode}).then(function(res) {
								console.log(res)
								if( res.data.code === 'success' ) {
									console.log(res.data.message)
										let deliveryAreaSubInfo = res.data.obj.deliveryAreaSubInfo;
										if( deliveryAreaSubInfo != null ) {
											deliveryAreaSubInfo.forEach((item,index) => {
												let treeData = {
													label: item.areaName,
													areaCode: item.areaCode,
