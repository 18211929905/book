			        ...
			        <div class="bootstrap-table bordered">
			            <div class="fixed-table-toolbar">
				            <div class="columns columns-right margin-bottom-10">
								   ...
								   <!--<button class="btn btn-success" title="更新" data-toggle="modal" data-target="#updata" @click="change()">编辑</button>-->
								...
								<table class="table table-striped table-hover table-bordered">
                                     <thead>
									     <tr class="text-center">
			            				    ...
			            				    <th class="text-center">采购区域编码</th>
			            				    <th class="text-center">采购区域名称</th>
			            				    <!--<th class="text-center">父级采购区域编码</th>
			            				    <th class="text-center">集联数据信息关系</th>-->
			            				    <th class="text-center">采购区域缩写</th>
			            				    <th class="text-center">采购区域业务值</th>
									 ...
									 <tbody class="text-center">
										 <tr v-for="(data,index) in datalist">
											 <td @click="check(index)"><input type="radio" v-model="radioValue" :value="index" name="radio"/></td>
						...
						<div class="modal-dialog modal-wide" >
							<div class="modal-content">
								<div class="modal-header">
									...
									<h4 class="modal-title text-center">新增采购区域管理</h4>
								</div>
											...
											<div class="form-body">
												<div class="row">
													<div class="col-md-12">
															...
															<div class="form-group">
																<label class="control-label col-md-5">采购区域缩写</label>
																<div class="col-md-7 ">
																		<input type="text" class="form-control" v-model="addDataParams.areaAddrName"/>
																</div>
															</div>
															<div class="form-group">
																<label class="control-label col-md-5">采购区域业务值</label>
																<div class="col-md-7 ">
					...
					<div class="modal fade" id="updata" aria-hidden="true">
						<div class="modal-dialog modal-wide">
							<div class="modal-content">
								<div class="modal-header">
									<button type="button" class="close" data-dismiss="modal" aria-hidden="true"></button>
									<h4 class="modal-title text-center">编辑采购区域管理</h4>
								</div>
								<div class="modal-body form clearfix">
									<div class="col-md-5 col-sm-6 tree-padding">
										<ZTree ref='tree'
	...
	export default {
		...
		data() {
			return {
				...
				treeRootCode: commonConfig.areaRoot.parea,    //点击树根节点获取的相关编码
				areaCode: '',
				checked: false,
				obj: {},
				radioValue: -1,
		...
		methods: {
			//初始化缓存
			init() {
				api.initPurchasingAreaToCache().then((res) => {
			...
			updata() {
                ...
                api.getUpdatePurchasingAreaInfo(_params).then(function(res) {
					  if( res.data.code === 'success' ) {
			...
			addData() {
				...
				let insertPromise = new Promise((resolve, reject) => {
					...
					api.getInsertPurchasingAreaInfo(_params).then((res) => {
						if( res.data.code === 'success' ) {
			...
			add() {
				...
				this.required = false
				this.getTree();
				let _params = commonConfig.area.parea;
				api.ordinalInfo.getSequence({"serviceCode":_params}).then((res) => {
					this.addDataParams.areaCode = res.data.obj;
				});
				this.addDataParams.fatherAreaCode = commonConfig.areaRoot.parea;
			},
			//封装 - 查询
			getQueryInfo(params) {
				this.radioValue = -1
				api.getQueryPurchasingAreaInfo(params).then((res) => {
					if(res.data.code === 'success') {
			...
			//根据行政区域编码返回其直属下级
			getTree() {
                let _params = commonConfig.areaRoot.parea;
				let _this = this;
				api.getPurchasingAreaInfoByAreaCode({"areaCode" : _params}).then(function(res) {
                      if( res.data.code === 'success' ) {
							if(res.data.obj) {
								_this.treeData = [];
								_this.options.rootName =  res.data.obj.areaName;
								let purchasingAreaSubInfo = res.data.obj.purchasingAreaSubInfo;
								if(purchasingAreaSubInfo) {
									purchasingAreaSubInfo.forEach((item,index) => {
										let treeData = {
											label: item.areaName,
			...
			change(id) {
				if( this.checked ) {
					...
					api.getQueryPurchasingAreaInfo({"areaCode":_params}).then((res) => {
			...
			getRootUpdata(rootCode) {
				this.updataParams.fatherAreaCode = commonConfig.areaRoot.parea;
			},
			getRootAdd(rootCode) {
				this.addDataParams.fatherAreaCode = commonConfig.areaRoot.parea;
			},
			...
			async loadingChild(node, index) {
				...
				let data = await new Promise((resolve, reject) => {
					console.log(node.areaCode)
						api.getPurchasingAreaInfoByAreaCode({"areaCode" : node.areaCode}).then(function(res) {
								console.log(res)
								if( res.data.code === 'success' ) {
