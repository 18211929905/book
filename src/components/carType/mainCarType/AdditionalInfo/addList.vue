...
export default {
    ...
    methods: {
                ...
                this.addList.forEach(item => {
                    for(let key in item) {
                        if(item.hasOwnProperty(key) && key !== "carDetailCode") {
                            let data = {
                                "addCode": key,
                                "addValue": item[key].trim(),
                                "carCode": this.carCode,
                                "carDetailCode": item.carDetailCode,
                                "id": item[`${key}id`] || null
                            }
                            if(key.indexOf('id') < 0){
                                if(this.propsList[key.replace(/[^0-9]/ig,"")]){
                                    data.addName = this.propsList[key.replace(/[^0-9]/ig,"")].refDetailName;
                                }
                            }
                            params.push(data)
                        }
        ...
        getParams() {
            return new Promise((resolve, reject) => {
                api.getDataDictionary({
                    ...
                    // if(msg.data.obj.referenceDetailInfos.length >){
        ...
        insertOrUpdate(params) {
            api.addCarMainInfo(params).then((res) => {
