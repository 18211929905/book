                                    ...
                                    <div class="row">
                                        <div class="pull-right">
                                            <div class="col-md-12">
                                                ...
                                                <input type="button" class="btn btn-primary" value="确定" @click="insert(0)" data-dismiss="modal" v-if="!myStatus" />
                                                <input type="button" class="btn btn-primary" value="确定" @click="insert(1)" data-dismiss="modal" v-if="myStatus" />
                                            </div>
                                        </div>
                                    </div>
                                </div>
...
export default {
  ...
  methods: {
    ...
    insert(e) {
      ...
      if (!e) {
        let insertPromise = new Promise((resolve, reject) => {
          ...
          api.carType.mainCarType.getInsert(_params).then(res => {
            if (res.data.code === "success") {
        ...
        let updataPromise = new Promise((resolve, reject) => {
          ...
          api.carType.mainCarType.getUpdata(_params).then(res => {
            if (res.data.code === "success") {
    ...
    getQueryInfo(_params, flag = false) {
      ...
      this.radioValue = -1
      this.carCode = ''
      api.carType.mainCarType.getQueryInfo(_params).then(res => {
        if (res.data.code === "success") {
      ...
      if (pageStart === 1) {
        api.carType.ioType.getQueryInfo(_params).then(res => {
          if (res.data.code === "success"){
            ...
            console.log('#####mainCarType/listbody.vue => getIoTypeQueryInfo方法请求数据失败')
          }
        });
      } else {
        api.carType.ioType.getQueryInfo(_params).then(res => {
