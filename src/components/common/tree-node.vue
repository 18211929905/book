<template>
    <ul>
        <li v-for='(item, index) of nodeData' v-show="!item.hasOwnProperty('visible') || item.visible" :key="item.key">
            <div :key="index" v-show="item.hasOwnProperty('dynamicAdd')">
                <input ref="inputadd" type="text" class="add-input" value="" v-focus :key="item.key" @blur="addNode(item, $event)" @keyup.enter="addNode(item, $event)">
            </div>
            <!--:class="{'node-selected':(item.checked && !options.showCheckbox) || item.searched }"  :class="{'node-selected':item.checked}"-->
            <div v-show="!item.hasOwnProperty('dynamicAdd')" class="item-handle-area" :class="{'node-selected':(item.checked && !options.showCheckbox) || item.searched }" @click="handleNode(item)">
                <i v-if=" isLeaf(item) " @click.stop='handleNodeExpand(item, index)' class="icon iconfont  handle-icon" :class="[ item.open ? options.iconClass.open : options.iconClass.close ]" :style="options.iconStyle"></i>
                <i v-else class="icon iconfont " :class="leafIcon(item)" :style="options.iconStyle"></i>
                <div class="inputCheck" :class="{notAllNodes:item.nodeSelectNotAll}" :style="{width:inputWidth+'px', height:inputWidth+'px', color: options.iconStyle.color}" v-if="options.showCheckbox" @click.stop="walkCheckBox(item)">
                    <input type="checkbox" class="check" v-if="options.showCheckbox  &&  !item.nodeSelectNotAll" :checked="item.checked" @change="checkBoxChange(item, $event)" :key="item.key" />
                </div>
                <span class="halo-tree-icon_loading halo-tree-iconEle" v-show="item.loading" :style="options.iconStyle">
                </span>
                <span class="label">
                ...
                <i class="iconfont add-icon" :class="options.iconClass.add" :style="options.iconStyle" v-if="canAdd(item)" @click.stop="handleClickAddNode(item, index)"></i>
            </div>
            <tree-node ref="treenode" v-if="item.children && item.children.length > 0" :options="options" @handlecheckedChange="handlecheckedChange" v-show='item.open' :tree-data="item.children">
            </tree-node>
        </li>
    </ul>
</template>
