<template lang="html">
  <modal :show.sync="show" :large="true" :backdrop="false" effect="fade" width="80%">
    <div slot="modal-header" class="modal-header">
      <h4 class="modal-title text-center">新增菜单</h4>
    </div>
    ...
    <div slot="modal-footer" class="modal-footer">
      ...
      <button type="button" class="btn btn-primary" @click="insertMenuInfo()">确定</button>
    </div>
  </modal>
</template>
    ...
    methods: {
      ...
      getSequence: function() {
        ...
        let params = {
          ...
          method: "POST",
          body: JSON.stringify({
            serviceCode: config.menu.serviceCode
          })
        }
        api.ordinalInfo
          .getSequence(params)
      ...
      getButtonTypes: function() {
         ...
         let params = {
          ...
          method: "POST",
          body: JSON.stringify({
            refCode: config.menu.buttonType.refCode
          })
        }
        api.menu
      ...
      show: {
        handler(val) {
          if (val === true) {
            ...
            this.$data.menu = {
              fatherMenuCode: config.menu.rootMenuCode,
              menuCode: '',
              releationCodeList: '',
              menuName: '',
              menuPath: '',
              needLogin: '0',
              isDisplay: 0,
              buttonType: config.menu.buttonType.default,
              remark: '',
              isDeleted: '0'
            }
            this.$data.treeData = []
